...
| Safety/resilience gates stay reproducible | Technical Proof shows the deterministic eval matrix; current baseline is 13/13 PASS including provider self-repair and a two-attempt read-only retry budget | `lib/agent-evals.ts`, `lib/provider-retry.ts`, `app/api/agent/evals/route.ts`, `components/agent-eval-panel.tsx` | `lib/agent-evals.test.ts`, `lib/provider-retry.test.ts`, `bun scripts/agent-evals.ts` |
