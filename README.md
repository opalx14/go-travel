...
| Judge readiness is mode-aware | Preflight combines evals, benchmark, tool manifest, Qwen health and Atlas CLI readiness; Demo can be READY_WITH_FALLBACK while Live is BLOCKED unless connected prerequisites pass | `lib/judge-preflight.ts`, `lib/qwen-health.ts`, `lib/atlas/readiness.ts`, `app/api/agent/preflight/route.ts`, `components/judge-preflight-panel.tsx` | `lib/judge-preflight.test.ts`, `/api/agent/preflight?mode=demo|live` |
